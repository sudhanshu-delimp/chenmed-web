...
<div id="main-content">
	...
	$policyprocedure = get_field('policyprocedure');
	$version_number = get_field('version_number');
	$policydate = get_field('policydate');
	$revisiondate = get_field('revisiondate');
	$reviewedby =  get_field('reviewedby');
	$approved =  get_field('approvedby');
	$revisionschedule =  get_field('revisionschedule');
	$purpose =  get_field('purpose');
	$termsdefinitions =  get_field('termsdefinitions');
	$reporting_requirements =  get_field('reporting_requirements');
	$statement =  get_field('policy_statement');
	$progressive =  get_field('progressive');
	$policynumber = get_field('policynumber');
							...
							<div class="post-wrap">
								<?php if (!extra_is_builder_built()) { ?>
									<div class="post-content entry-content">
										<?php
										if ($subject || $policyprocedure || $policynumber || $version_number || $policydate || $revisiondate || $reviewedby || $approved || $revisionschedule) {
										echo '<table>
                                    <tbody>
                                    <tr>
                                    <td><h5> Subject:</h5> ' . ($subject ? $subject : "") . '</td>
                                    <td><h5> Policy/Procedure:</h5> ' . ($policyprocedure ? $policyprocedure : "") . '</td>
                                    <td><h5> Policy #:</h5> ' . ($policynumber ? $policynumber : "") . '</td>
                                    <td><h5> Version #:</h5> ' . ($version_number ? $version_number : "") . '</td>
                                    <td><h5> Policy Inception Date:</h5> ' . ($policydate ? $policydate : "") . '</td>
                                    </tr>
                                    <tr>
                                    <td><h5> Last Revisions Date:</h5> ' . ($revisiondate ? $revisiondate : "") . '</td>
                                    <td><h5> Reviewed By:</h5> ' . ($reviewedby ? $reviewedby : "") . '</td>
                                    <td><h5> Approved By:</h5> ' . ($approved ? $approved : "") . '</td>
                                    <td><h5> Revision Schedule:</h5> ' . ($revisionschedule ? $revisionschedule : "") . '</td>
                                    </tr>
                                    </tbody>
                                    </table>';
										}

...
<div id="main-content">
	...
	$requirements = get_field('requirements_and_resources');
	$attestation = get_field('attestation');
	$revised = get_field('date_revised');
	$version =  get_field('version');
	$approved =  get_field('approved_by');
	$coe =  get_field('coe');
	$date =  get_field('date_approved');
	$written =  get_field('written_by');
	$standards = get_field('standards');
	$rolefunction =  get_field('rolefunction');
	$url = get_field('url');
							...
							<div class="post-wrap">
								<?php if (!extra_is_builder_built()) { ?>
									<div class="post-content entry-content">
										<?php
									if($version || $date || $revised || $approved || $written || $coe || $rolefunction) {
										echo '<table>
                                    <tbody>
                                    <tr>
                                    <td><h5> Version #: </h5>' . ($version ? $version : "") . '</td>
                                    <td><h5> Date Approved: </h5>' . ($date ? $date : "") . '</td>
                                    <td><h5> Date Revised: </h5>' . ($revised ? $revised : "") . '</td>
                                    <td><h5> Approved By: </h5>' . ($approved ? $approved : "") . '</td>
                                    </tr>
                                    <tr>
                                    <td><h5> Written By: </h5>' . ($written ? $written : "") . '</td>
                                    <td><h5> COE: </h5>' . ($coe ? $coe : "") . '</td>
                                    <td><h5> Role/Function: </h5>' . ($rolefunction ? $rolefunction : "") . '</td>
                                    <td>&nbsp;</td>
                                    </tr>
                                    </tbody>
                                    </table>';
									...
									if($standards){
										echo "<h5>Standards</h5>";
										echo "<hr>";
										echo $standards;
										echo "<br><br>";
									}
									if($requirements){
										echo "<h5>Requirements & Resources</h5>";
										echo "<hr>";
										echo $requirements;
										echo "<br><br>";
									}
